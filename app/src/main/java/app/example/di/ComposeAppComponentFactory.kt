...
import dev.zacsweers.metro.Provider // Changed import
import kotlin.reflect.KClass // Import for KClass
...
class ComposeAppComponentFactory : AppComponentFactory() {
     ...
     */
    private inline fun <reified T : Any> getInstance( // Added : Any constraint for T
        classLoader: ClassLoader,
        className: String,
        providers: Map<KClass<out T>, @JvmSuppressWildcards Provider<T>>, // Key type KClass, Provider type Metro
    ): T? {
        // Load the class using the provided ClassLoader and attempt to retrieve the instance.
        val clazz = Class.forName(className, false, classLoader).kotlin // Get KClass
        val modelProvider = providers[clazz] ?: return null
        return modelProvider() as T // Use function call syntax for provider
    }

    /**
     * Called by the Android system to instantiate an activity. This method checks if the
     * activity can be provided by the Metro graph. If the Metro graph can provide
     ...
     * initializes the Metro graph and retrieves the map of activity providers,
    ...
    override fun instantiateApplicationCompat(
        ...
        activityProviders = (app as CircuitApp).appComponent().activityProviders // appComponent() now returns AppGraph
        return app
    }
     ...
     * upon instantiation.
     *
     * This map is initialized when the application is created via the Metro graph.
     */
    companion object {
        private lateinit var activityProviders: Map<KClass<out Activity>, Provider<Activity>> // Key type KClass, Provider type Metro
    }
}
